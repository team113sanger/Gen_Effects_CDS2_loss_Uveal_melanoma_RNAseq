...
2. **Data Pre-processing**: This section includes the alignment to the required reference genomes, GRCh38 for Human alongside ENSEMBLv103 annotation and [NOD_ShiLtJ_V1_PDX](./reference/NOD_ShiLtJ_V1_PDX_ref/README.md) customised mouse reference with annotations from ENSEMBL v107; to filter mouse reads from the Human mapped BAM files using **XenofilteR**. Alignment against the Human reference genome was performed in the same way for both datasets. Instructions on how the custom mouse reference and analyses at these stage were performed can be found in the [README here](./documentation/Alignment_and_Filtering_of_mouse_reads_wXenofilteR.md). 
  
3. **Data analysis - Expression quantification and differential gene expression**: This section delineates the parameters used for the transcript quantification analysis.  Instructions on how the quantification was performed can be found in the [README here](./documentation/Transcript_quantification_analysis.md).
...
- The following software was used:
  ...
  - ENSEMBL VEP version `107`[**here**](http://jul2022.archive.ensembl.org/index.html)
