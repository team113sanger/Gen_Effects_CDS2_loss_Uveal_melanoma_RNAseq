...
option_list<- list(
  ...
  make_option(c("--pdxID"), action="store_true", type="character", default=NA, help="Character string that if present in sample name it will mark them as Xenograft"),
...
#Verify there is a marker for PDX samples 
if(is.na(opt$pdxID)){
  stop(paste0("NO character string provided to mark PDX samples, e.g. CDS2"))
}
...
for (i in 1:dim(manif)[1]){
  ...
  #bsub -q normal -M 8000 -R'select[mem>8000] rusage[mem=8000] span[hosts=1]' -n 4 -o 6352_project_tests/logs/cramtofastqs/cramtofq_21.o -e 6352_project_tests/logs/cramtofastqs/cramtofq_21.e 'iget /illumina/runs/38/38900/lane1/plex21/38900_1#21.cram - | samtools-1.13/bin/samtools collate -u --threads 4 -O - |/software/CASM/modules/installs/samtools/samtools-1.13/bin/samtools fastq --threads 4 -1 6352_project_tests/fastqs/6352STDY10233950_R1.fastq.gz -2 6352_project_tests/fastqs/6352STDY10233950_R2.fastq.gz '
  if(is.na(opt$ref)){
...
#############################################################################################
# 3. Add information to the manifest   ------------------
manif$donor_id<- substr(manif$sample_donor_id, start = 1,stop =(nchar(manif$sample_donor_id) -7))
manif$Proc_as_PDX<- ifelse(grepl(opt$pdxID, manif$sample_donor_id), "Y", "N")
